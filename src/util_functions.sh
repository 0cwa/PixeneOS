...
function prepare_boot_animation_module() {
  local helper_root="${1}"
  local payload_paths payload_path dark_payload_path init_file registry_file module_source

  if [[ "${ADDITIONALS[BOOT_ANIMATION]}" != 'true' ]]; then
    return 0
  fi

  payload_paths="$(_boot_animation_payload_paths)" || return 1
  payload_path="${payload_paths%%
...
function verify_boot_animation_ota() {
  local ota_path="${1}"
  local temp_dir payload_paths payload_path dark_payload_path avbroot_bin afsr_bin ota_abs
  ...
  payload_paths="$(_boot_animation_payload_paths)" || return 1
  payload_path="${payload_paths%%  temp_dir="$(mktemp -d "${WORKDIR}/boot-animation-verify.XXXXXX")" || return 1
...
\n'*}"
  dark_payload_path="${payload_paths#*
  init_file="${helper_root}/lib/modules/__init__.py"
  registry_file="${helper_root}/lib/modules/registry.py"
  module_source="${helper_root}/lib/modules/boot_animation.py"
  if [[ ! -f "${init_file}" || -L "${init_file}" ]]; then
...
\n'*}"
  dark_payload_path="${payload_paths#*  temp_dir="$(mktemp -d "${WORKDIR}/boot-animation-verify.XXXXXX")" || return 1
  temp_dir="$(realpath -- "${temp_dir}")" || return 1
  ota_abs="$(realpath -- "${ota_path}")" || {
    rm -rf -- "${temp_dir}"
    return 1
